...
def count_branch(nx_g, thres):
    # Input a path(list of nodes), a nx_g
    # Randomly select k patches that fit the condition, and resample a piece of seq.
...
def sample(nx_graph, graph_out_pref):
    ...
    thres_list = [0.1 + 0.01*i for i in range(41)]  #step up weight from 0.1 to 0.4

    branch_f = graph_out_pref + "branch_01-05-no-homo.fa"
    n_branch = [] #store the number of actual changing sites given different threshold.
    
    with open(branch_f, 'w') as out, open(branch_f + '.path', 'w') as path_out:   
        for thres in thres_list:
            ...
            print("Number of initial candidate nodes: ", len(candi_loc))
            candi_on_bb = []
            ...
            print("second round sites:", len(candi_rd2), candi_rd2)
            print("new thres:", thres * tolerance)
            ...
            for patch in patch_list_rd2:
                if len(patch) > 2: #only add the patch longer than before
                    ...
                    if patch[0] in patched_bb_2rd and patch[-1] in patched_bb_2rd:
                        ...
                        #patched_bb[idx1 - 1 : idx2] = patch
                        print((idx1, idx2), patched_bb_2rd[idx1 : idx2 + 2], len(patched_bb_2rd))
            ...
            for i, patched_bb in enumerate(patched_bb_list):
                ...
                out.write(seq+'\n')
                #path_out.write(">" + patched_bb[0] + '\n') 
                #path_out.write(str(patched_bb[1]) + "\n")
