...
def Grouped_beam(nx_g, beam_wid, path_out_f):
    ...
    for src_nd in src_group: 
        ...
        if nx_graph.edges[0, src_nd]['weight'] < 1 or src_nd >= 500:
            cnt -= 1
    print("totol out_nodes: %s;valid group:%s"%(len(src_group), cnt))
    ...
    print("number of groups(from src):", len(src_group), "k in each=", sub_width)
    ...
    with open(out_f, 'w') as out, open(out_f + ".path", 'w') as path_out:
        ...
        for i, path_grt in enumerate(path_grt_list):
            for j, path in enumerate(path_grt):
                ...
                out.write(seq+'\n')
                # output the node list in the paths
...
if __name__ == '__main__':
    # set paras from different num
    num = 6
    k = 500
    pref = "../../data/2_geno_mix/mix_" + str(num) + "/"
    ec_reads = pref + "mix_" + str(num) +"_ec.fa"
    ass_reads = pref + "mix_" + str(num) +"_ass.fa"
    """
    pref = "../../data/1_geno_canu/gen_" + str(num) + "/"
    ec_reads = pref + "gen_" + str(num) +"_ec.fa"
    ass_reads = pref + "gen_" + str(num) +"_ass.fa"
    """
    graph_path = pref + \
                     (os.path.splitext(os.path.basename(ec_reads))[0]) + "_ON_" +\
                     (os.path.splitext(os.path.basename(ass_reads))[0]) + ".graph"
    print("loading graph from:\n", graph_path)                     
    nx_graph = utils.load_nx_graph(graph_path)
    Grouped_beam(nx_graph, k, pref + "_beam_" + str(k) + ".fa")
